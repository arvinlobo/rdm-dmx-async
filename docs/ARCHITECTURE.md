...
(as do `examples/` and `hardware_tests/`) and is not part of this layering. The
packaged CLI is `rdm_dmx_async/cli.py` and belongs to the application layer below.
See `api/app.py`; its dependencies require the `api` extra (`uv sync --extra api`).
...
flowchart TD
    ...
    L6["6. Device Service Layer<br/>RdmDevice, DeviceRepository, DiscoveryService,<br/>device_apis/* (18 PID-specific API modules)"]
    ...
    L2["2. Transport Layer<br/>AsyncSerialTransport, FrameBuffer, RdmProtocolDetector,<br/>InterfaceAdapter: Enttec, DMXKing (stub), GenericSerial, BareUsbRs485"]
    L1["1. Hardware / OS Layer<br/>pyserial, asyncio, OS serial drivers"]

    L7 --> L6 --> L5 --> L4 --> L3 --> L2 --> L1
```

  The arrows show the primary top-down request flow, not an exclusive import
  graph. `NetworkManager` is the composition root and directly constructs objects
  from several lower layers; `RDME120Protocol` also communicates directly with
  `AsyncTransport` while coordinating RDM windows with `DmxFrameScheduler`.
...
- Concrete adapters (`transport/adapters/`):
  ...
  - `BareUsbRs485Adapter`: Bare USB-RS485 (FTDI FT232R) interfaces with no onboard framing microcontroller - the host toggles the UART BREAK condition itself
...
- Configurable request/response timeout handling; transaction policies use
  longer application-level timeouts to accommodate serial adapters and retries
...
> **Note**: There are two distinct correlation components:
> `protocols/response_correlator.py::ResponseCorrelator` matches responses to
> in-flight requests via futures, while
> `transaction/correlator.py::LateResponseClassifier` classifies whether a
> response belongs to the current or a previous retry attempt.
...
- `device_apis/*`: One module per PID group, each exposing a focused API (`DeviceControlAPI`, `DeviceInfoAPI`, `DeviceLabelAPI`, `DeviceMaintenanceAPI`, `DisplaySettingsAPI`, `DmxConfigAPI`, `DmxModesAPI`, `DmxSlotsAPI`, `LampControlAPI`, `PositionConfigAPI`, `PowerControlAPI`, `PresetControlAPI`, `ProxyAPI`, `RawPidAPI`, `SelfTestAPI`, `SensorDefinitionsAPI`, `SensorsAPI`, `SystemInfoAPI`)
...
- `services/device_apis/` (18 modules, one per PID group)
...
- `InterfaceAdapter` is an abstract base, extended by `EnttecAdapter`, `BareUsbRs485Adapter`, `GenericSerialAdapter`, and `DMXKingAdapter`
- New hardware can be added without modifying existing transport or protocol code
- Retry behavior is configured with `RetryPolicy` values and reusable policy instances

### Liskov Substitution Principle (LSP)

- Transport and protocol consumers target `AsyncTransport` and `RdmProtocol`
  contracts rather than concrete implementations
- Adapter capabilities are not identical: `DMXKingAdapter` is currently a
  stub, and `GenericSerialAdapter` does not implement DMX512 output
- Unsupported adapter operations fail explicitly with `NotImplementedError`

### Interface Segregation Principle (ISP)

Separate contracts and focused components are used for different concerns:
...
- High-level modules depend on abstractions:
  ...
  - Service-layer objects depend on the `RdmProtocol` structural interface
  - `NetworkManager` composes the concrete transport, scheduler, protocol,
    repository, and discovery services at the application boundary
...
- `RetryPolicy`: Configurable retry behavior passed into transactions
...
- `EnttecAdapter`, `BareUsbRs485Adapter`, `GenericSerialAdapter`: Adapt their respective wire protocols
...
sequenceDiagram
    ...
    participant API as DeviceLabelAPI
    ...
    User->>App: discover/select device
    App-->>User: RdmDevice
    User->>API: device.device_label.get()
    API->>Svc: get_string_parameter(DEVICE_LABEL)
    Svc->>Svc: check parameter cache
    Svc->>Txn: get(uid, pid, timeout)
    Txn->>Proto: send_get_command(uid, pid, txn_number)
    Proto->>Proto: encode with PacketEncoder
    Proto->>Sched: pause DMX for RDM window
    Proto->>Trans: send(encoded RDM packet)
    Trans->>Adapter: frame_rdm_request(packet)
    Trans->>HW: write framed bytes
    HW-->>Trans: raw response bytes
    Trans->>Adapter: find frame and parse response
    ...
    Svc->>Svc: cache response data
    Svc-->>API: decoded string
    API-->>User: label
```
...
- ✅ All 7 layers are implemented; pytest collects 287 automated tests when the `api` extra is installed (see `tests/`)
- ✅ Enttec USB PRO and bare USB-RS485 (FTDI FT232R) interfaces are implemented, with dedicated adapter and hardware test programs
...
- ⚠️ `GenericSerialAdapter` explicitly does not support DMX512 output (`frame_dmx_output` raises `NotImplementedError`) - it is not currently a substitute for `BareUsbRs485Adapter`

## Next Steps

1. **DMXKing support**: Implement `DMXKingAdapter`'s framing, parsing, and frame-length detection (currently all stubbed)
2. **Adapter consolidation**: Evaluate whether `BareUsbRs485Adapter`'s raw DMX/RDM framing can be folded into `GenericSerialAdapter` as an additional `FramingMode`, or whether they should remain separate given `GenericSerialAdapter`'s current DMX-output restriction
